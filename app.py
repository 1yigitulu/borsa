# app.py
# Kişisel Yapay Zeka Borsa Asistanı v7.3 (Final Kararlı Sürüm)
# Özellikler: Şifreleme, Önbellek, Mentor Yorumları, Mobil PDF Çıktısı
...
# === 0. YARDIMCI FONKSİYONLAR (PDF & KARAKTER) ===
def tr_to_en(metin):
...
def pdf_olustur(df):
    """fpdf2 kütüphanesi ile mobil uyumlu PDF üretir."""
    pdf = FPDF()
    pdf.add_page()
    ...
    # Saf bayt olarak döndür (Mobilde açılma garantisi)
    return bytes(pdf.output())

# === 1. GÜVENLİK (LOGIN EKRANI) ===
def sifre_kontrol():
    if st.session_state.get("password_correct", False):
        return True

    # Sayfa ayarını ilk burada yapıyoruz (Streamlit kuralı)
    st.set_page_config(page_title="Giriş Yap", page_icon="🔒")
    st.header("🔒 Korumalı Alan")
    
    SIFRE = "1239" # İstediğin şifre
...
# Giriş yapıldıysa sayfa başlığını ana uygulamaya çevir
st.set_page_config(
...
# === 3. GÜVENLİ VERİ ÇEKME (CACHE SİSTEMİ) ===
...
def yorum_olustur(sembol, hisse_df, tahmin, guven, features):
    ...
    if tahmin == 2:
        ...
        * **💰 Çıkış:** Beklenen kâr (örneğin %5) gelirse bekleme, kârı al.
        ...
        st.error(f"**🛑 SATIŞ BASKISI / UZAK DUR**\n\nModel zayıflık tespit etti.")
    else:
        st.warning(f"**⚪ KARARSIZ BÖLGE (Nötr)**\n\nBekleyip görmek en iyisi.")

# === 6. ARAYÜZ ===
st.title("🦅 Yapay Zeka Borsa Asistanı v7.3")
...
with tab1:
    ...
    sembol_giris = st.text_input("Hisse Kodu (Örn: THYAO):", "").upper()
